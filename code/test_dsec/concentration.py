...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="DAV2 checkpoint path (required by model init).",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def main() -> None:
    ...
    concentrator = ConcentrationDav2(
    ...
    if args.concentrator_checkpoint:
        ...
        concentrator.concentrator.load_state_dict(concentrator_state, strict=True)

    concentrator_rgb = concentrator.concentrator(events)  # (B,3,H,W)
    depth_pred = dav2(concentrator_rgb).squeeze(1)
