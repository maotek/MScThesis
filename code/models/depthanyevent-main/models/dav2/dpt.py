...
class DepthAnythingV2(nn.Module):
    def __init__(
        ...
        self.depth_head = DPTHead(self.pretrained.embed_dim, features, use_bn, out_channels=out_channels, use_clstoken=use_clstoken, activation=activation)
    ...
    def forward(self, x):
        ...
        depth, full_features = self.depth_head(features, patch_h, patch_w)
        ...
        return depth, full_features
    
    def infer_image(self, raw_image, input_size_width=None, input_size_height=None, extract_features = False):
        ...
        depth, full_features = self.forward(image)
        
        depth = F.interpolate(depth, (h, w), mode="bilinear", align_corners=False)

        if extract_features:
            full_features = F.interpolate(full_features, (h, w), mode="bilinear", align_corners=False)
            return depth, full_features
        return depth
