    ...
    def forward(self, out_features, patch_h, patch_w):
        ...
        for i, x in enumerate(out_features):
            if self.use_clstoken:
                ...
                x = x[0]
            print(f"Feature {i} shape before projection: {x.shape}")
            x = x.permute(0, 2, 1).reshape((x.shape[0], x.shape[-1], patch_h, patch_w))
            print(f"Feature {i} shape after reshaping: {x.shape}")
            x = self.projects[i](x)
            print(f"Feature {i} shape after projection: {x.shape}")
            x = self.resize_layers[i](x)
            print(f"Feature {i} shape after resizing: {x.shape}")
            
            out.append(x)
        # print("out features shapes after projection and resizing:", [o.shape for o in out])
